...
/* !!!!!
 * To ENABLE animation, go to line 163 and uncomment the code: "ui.setState(field);"
 */

/**
 * This class includes the methods to support the search of a solution.
 */
public class SearchRecrusion {
	static int[] dimensions = askUserPentominoDimensions();
	public static final int horizontalGridSize = dimensions[1];
	public static final int verticalGridSize = dimensions[0];
	public static char[] input = { 'W', 'I', 'T', 'Z', 'L' };
	public static UI ui = new UI(horizontalGridSize, verticalGridSize, 50);

	public static void search() {
		Scanner scan = new Scanner(System.in);
		// create a new field filled with -1's
		...
		if (backtrackSearch(field, 0)) { // if solution is found
			System.out.print("Solution found! Press Enter to see time taken: ");
			String enter = scan.nextLine();
		} else { // if solution is not found
			System.out.println("Solution not found! Press Enter to end.");
			String enter = scan.nextLine();
			System.exit(0);
		}
	...
	/**
	 * Recrusive function that that searches for a solution for placing pentominos
	 * in a field. The method uses backtracking to check possible place, add piece,
	 * and remove piece from field.
	 * 
	 * @param field      a matrix representing the board to be fulfilled with
	 *                   pentominoes
	 * @param pieceIndex index of current pentomino piece being placed
	 * @return
	 */
	private static boolean backtrackSearch(int[][] field, int pieceIndex) {

		// return true if the pentomino can fit in
		if (pieceIndex == input.length) {
			ui.setState(field);
			return true; // if a solution is found
		}

		int pentID = characterToID(input[pieceIndex]);
		...
		for (int i = 0; i < pieces.length; i++) {
			...
			for (int x = 0; x < field.length; x++) {
				for (int y = 0; y < field[x].length; y++) {
					if (input[pieceIndex] != 'X' || !isCornerPosition(field, piece, x, y)) { // if pentomino input is x
																								// and x is NOT at the
																								// corner
						if (checkPossiblePlace(field, piece, x, y)) { // check if possible (overlapping & out of bounds)
							addPiece(field, piece, pentID, x, y); // add the piece if possible
							if (backtrackSearch(field, pieceIndex + 1)) { // recrusive function
								return true; // if a solution is found
							}
							removePiece(field, piece, x, y); // remove piece
						}
					}
				}
			}
	...
	/**
	 * 
	 * Method to check if a pentomino piece is positioned at the corner
	 * 
	 * @param field          a matrix representing the board to be fulfilled with
	 *                       pentominoes
	 * @param pentominoPiece the pentomino piece that is being placed
	 * @param x              x-coordinate of the field
	 * @param y              y-coordinate of the field
	 * @return if positioned in corner or not
	 */
	private static boolean isCornerPosition(int[][] field, int[][] pentominoPiece, int x, int y) {
		return (x == 0 || x == field.length - 1) && (y == 0 || y == field[0].length - 1);
	}
	...
	public static boolean checkPossiblePlace(int[][] field, int[][] pentominoPiece, int x, int y) {
		for (int i = 0; i < pentominoPiece.length; i++) { // row of pentominoPiece
			for (int k = 0; k < pentominoPiece[i].length; k++) { // column of pentominoPiece
				// check if the indices are within the bounds of the field
				if (x + i >= 0 && x + i < field.length && y + k >= 0 && y + k < field[0].length) {
					// check for overlapping
					if (pentominoPiece[i][k] == 1 && field[x + i][y + k] != -1) {
						return false; // return false if overlapping, you cant place the piece at this position
					}
				} else {
					return false; // return false if out of bounds, you cant place the piece at this position
				}
				// ui.setState(field); // uncomment this line
			}
		}
		return true; // return true if no overlap and within bounds, you can place the piece
	}

	/**
	 * Adds the pentomino piece into matrix.
	 * 
	 * @param field   a matrix representing the board to be fulfilled with
	 *                pentominoes
	 * @param piece   a matrix representing the pentomino to be placed in the
	 *                board
	...
	/**
	 * Removes the pentomino from the matrix
	 * 
	 * @param field a matrix representing the board to be fulfilled with
	 *              pentominoes
	 * @param piece a matrix representing the pentomino to be placed in the
	...
	private static void removePiece(int[][] field, int[][] piece, int x, int y) {
		for (int i = 0; i < piece.length; i++) {
			for (int j = 0; j < piece[i].length; j++) {
				if (piece[i][j] == 1) {
					field[x + i][y + j] = -1; // replace it with -1 (empty space)
				}
			}
		}
	}
